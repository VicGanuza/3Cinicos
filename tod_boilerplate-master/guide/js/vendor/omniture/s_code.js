...
function s_doPlugins(s) {
	...
	//Page Name
	//s.pageName = s.eVar7 = document.location.pathname;
	if(!s.pageName)
		s.pageName = s.eVar7 = getPageName(document.URL);
	...
	if(dllink){
		s.prop31 = s.pageName;
		//s.prop33 = s.eVar31 = dllink;
		s.prop33 = s.eVar31 = dllink.substring(dllink.lastIndexOf("/")+1,dllink.length); 
		s.prop32 = s.eVar32 = dllink.substring(dllink.lastIndexOf(".")+1,dllink.length);
		s.events=s.apl(s.events,'event3',',',2);
		s.linkTrackVars=s.apl(s.linkTrackVars,'prop31,prop33,prop32,eVar32,eVar31,events',',',2);
		if(s.campaign){
			s.eVar33=s.campaign;
	...
	if(s_hierStr){
		if(s_hierStr[1]){
			s.hier1=s_hierStr[1].toLowerCase();
		}
	}
...
function getPageName(URL){
	...
	// Break down the URL into separate components using "/" as a separater 
	...
	for (var i=0; i < pageNameString.length; i++){ 
		...
		if (pageNameString[i] == "Pages"||pageNameString[i] == "http:"||pageNameString[i] == "https:"||pageNameString[i] == "")
		continue;
	else{
